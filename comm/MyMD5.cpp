...
void MD5Update(MD5_CTX1 *context, unsigned char* input, unsigned int inputlen) {
	...
	context->count[1] += inputlen >> 29;//先左移3位，求出位数，再右移32位，求出左32位
	if (inputlen >= partlen) {
...
//32位md5
void MD5Final(MD5_CTX1* context, unsigned char digest[16]) {
	...
	MD5Update(context, bits, 8);//在这里调用时，index=0；
